...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    if args.dataset == 'r4r':
        ...
        best_val = {'val_unseen': {"spl": 61.49, "sr": 67.31, "state":""}}
    ...
    for idx in range(start_iter, start_iter+args.iters, args.log_every):
        ...
        if aug_env is None:
            ...
            listner.vln_bert_scheduler.step();
            listner.critic_scheduler.step();
        else:
            jdx_length = len(range(interval // 2))
            print("第%d个epoch的学习率:%.10f" % (iter, listner.vln_bert_optimizer.param_groups[0]['lr']))
        ...
        if default_gpu:
            ...
            write_to_record_file(
                ('%s (%d %d%%) %s' % (timeSince(start, float(iter-start_iter)/args.iters), iter, float(iter)/args.iters*100, loss_str)),
                record_file
            )

...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None):
        ...
        # print("query layers",query_layer.shape)
...
class LxmertEncoder(nn.Module):
    def __init__(self, config):
        ...
        #     [
        #         EncoderLayer(
        #             AttentionLayer(ProbAttention(False, config.factor, attention_dropout=config.Inform_dropout, output_attention=config.output_attention), 
        #                         config.d_model, config.n_heads, mix=False),
        #             config.d_model,#注意是输入序列的长度减半，不是数据的d_model的大小减半;
        #             config.d_ff,
        #             dropout=config.Inform_dropout,
        ...
        #         ConvLayer(
        #             config.d_model
        #         ) for l in range(config.distill_layers)#for l in range(config.e_layers-1)
        #     ] ,
...
class NavCMT(BertPreTrainedModel):
    ...
    def forward(self, mode, txt_ids=None, txt_embeds=None, txt_masks=None,
        ...
        if mode == 'language':
            ...
            # print("input mask:",txt_masks.shape)
            # print("input txt:",txt_ids.shape)
            txt_token_type_ids = torch.zeros_like(txt_ids)
            txt_embeds = self.embeddings(txt_ids, token_type_ids=txt_token_type_ids)
            # print("text embeding:",txt_embeds.shape)
            for layer_module in self.encoder.layer:
                temp_output = layer_module(txt_embeds, extended_txt_masks)
                txt_embeds = temp_output[0]
            # txt_embeds=self.encoder.layer(txt_embeds,padding_attn_mask=extended_txt_masks)[0]
            # print("transformer output:",txt_embeds.shape)
        ...
        # cross-modal encoding per step
            ...
            # print("image feature:",ob_img_feats.shape);
            # print("angle feature:",ob_img_feats.shape);
            ...
            # print("image embedding",ob_embeds.shape);
            ...
            # print("image output:",ob_embeds.shape);
            ...
            # print("cross-modal:",hist_ob_embeds.shape)

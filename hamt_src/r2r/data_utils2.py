...
class ImageFeaturesDB(object):
    def __init__(self, img_ft_file, image_feat_size, img_aug_ft_file=None,img_aug_ft_file1=None,img_aug_ft_file2=None):
        self.image_feat_size = image_feat_size
        self.img_ft_file = img_ft_file
        self.img_aug_ft_file = img_aug_ft_file
        self.img_aug_ft_file1=img_aug_ft_file1;
        self.img_aug_ft_file2=img_aug_ft_file2;
        self._feature_store = {}
        if img_aug_ft_file:
            self._feature_store_aug = {}
            self._feature_store_aug1={}
            self._feature_store_aug2={}
        else:
            self._feature_store_aug = None
            self._feature_store_aug1=None;
            self._feature_store_aug2=None;
    ...
    def get_image_feature(self, scan, viewpoint):
        ...
        if self.img_aug_ft_file:
            ...
            if ran > 0.8:
                ...
                if key in self._feature_store_aug2:
                    fts = self._feature_store_aug2[key]
                else:
                    with h5py.File(self.img_aug_ft_file2, 'r') as f4:
                        fts = f4[key][...].astype(np.float32)[:,:self.image_feat_size]
                    self._feature_store_aug2[key] = fts
            elif ran >0.4:
                if key in self._feature_store_aug1:
                    fts = self._feature_store_aug1[key]
                else:
                    with h5py.File(self.img_aug_ft_file1, 'r') as f3:
                        fts = f3[key][...].astype(np.float32)[:,:self.image_feat_size]
                    self._feature_store_aug1[key] = fts
            else:
    ...
    for i, item in enumerate(load_instr_datasets(anno_dir, dataset, splits)):
        if dataset == 'rxr':
            ...
            for j, instr in enumerate(item['instructions']):
                ...
                # ''' BERT tokenizer '''
                new_item["instr_encoding"]=tokenizer.encode(instr)[:max_instr_len]
...
def get_point_angle_feature(sim, angle_feat_size, baseViewId=0, minus_elevation=False):
    ...
    for ix in range(36):
        ...
        # print(state.heading/math.pi*180.0)

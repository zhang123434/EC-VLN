...
class Seq2SeqCMTAgent(BaseAgent):
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        for t in range(self.args.max_action_len):
            ...
            if train_ml is not None:
                # Supervised training 使用的是有监督的训练
    ...
    def rollout1(self, train_ml=None, train_rl=True, reset=True):
        """
        :param train_ml:    The weight to train with maximum likelihood
        :param train_rl:    whether use RL in training
        :param reset:       Reset the environment
        ...
        # Language input
        txt_ids, txt_masks, txt_lens = self._language_variable(obs)
        # print("txt input:",txt_ids.shape);
        # print("txt input mask:",txt_masks.shape)
        ''' Language BERT '''
        language_inputs = {
            'mode': 'language',
            'txt_ids': txt_ids,
        ...
        txt_embeds = self.vln_bert(**language_inputs)
        # print("languaged encoder output",txt_embeds.shape)
        # Record starting point
        traj = [{
            'instr_id': ob['instr_id'],
            'path': [(ob['viewpoint'], ob['heading'], ob['elevation'])],

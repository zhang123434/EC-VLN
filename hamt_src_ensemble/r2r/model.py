...
class AttnDecoderLSTM(nn.Module):
    ...
    def __init__(self, embedding_size, hidden_size,
        ...
        self.embedding = nn.Sequential(
            nn.Linear(args.angle_feat_size1, self.embedding_size),
            nn.Tanh()
        )
        self.drop = nn.Dropout(p=dropout_ratio)
    ...
    def forward(self, action, feature, cand_feat,
        ...
        action: batch x angle_feat_size1
        feature: batch x 36 x (feature_size + angle_feat_size1)
        cand_feat: batch x cand x (feature_size + angle_feat_size1)
        ...
        if not already_dropfeat:
            ...
            feature[..., :-args.angle_feat_size1] = self.drop_env(feature[..., :-args.angle_feat_size1])   # Do not drop the last args.angle_feat_size1 (position feat)
        ...
        if not already_dropfeat:
            cand_feat[..., :-args.angle_feat_size1] = self.drop_env(cand_feat[..., :-args.angle_feat_size1])
...
class SpeakerEncoder(nn.Module):
    ...
    def forward(self, action_embeds, feature, lengths, already_dropfeat=False):
        ...
        x = action_embeds
        if not already_dropfeat:
            x[..., :-args.angle_feat_size1] = self.drop3(x[..., :-args.angle_feat_size1])            # Do not dropout the spatial features
        ...
        if not already_dropfeat:
            feature[..., :-args.angle_feat_size1] = self.drop3(feature[..., :-args.angle_feat_size1])   # Dropout the image feature

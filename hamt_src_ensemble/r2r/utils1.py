...
def load_nav_graphs(scans):
    ...
    graphs = {}
    for scan in scans:
        with open('/root/mount/Matterport3DSimulator/VLN-trans-main/VLN-trans/connectivity/%s_connectivity.json' % scan) as f:
            G = nx.Graph()
            positions = {}
            data = json.load(f)
...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        # if "/" not in split:
        with open('/root/mount/Matterport3DSimulator/EnvEdit-main/r2r_new_fine/R2R_%s.json' % split) as f:
...
def read_img_features(feature_store, test_only=False):
    ...
    if "detectfeat" in args.features1:
        views = int(args.features1[10:])
    else:
        views = 36
...
def angle_feature(heading, elevation):
    ...
    return np.array([math.sin(heading), math.cos(heading),
                     math.sin(elevation), math.cos(elevation)] * (args.angle_feat_size1 // 4),
                    dtype=np.float32)
...
def get_point_angle_feature(sim,baseViewId=0):

    feature = np.empty((36, args.angle_feat_size1), np.float32)
...
def get_all_point_angle_feature(sim):
    return [get_point_angle_feature(sim,baseViewId) for baseViewId in range(36)]
...
def ndtw_initialize():
    ndtw_criterion = {}
    scan_gts_dir = '/root/mount/Matterport3DSimulator/VLN-trans-main/VLN-trans/r2r_src_navigator/data/id_paths.json'
